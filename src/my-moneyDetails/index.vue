<template>
    <div class="moneyDetail">
         ...
         <div class="litItem" v-for="(item,index) in list" :key="index">
             ...
             <div class="right">
                 <div>{{item.is_income==1?'+':'-'}} {{item.money}}</div>
             </div>
         </div>
    </div>
</template>
...
import { Toast } from 'mint-ui';
    ...
    export default {
        ...
        methods:{
            ...
            initData(){
                var that = this
                apiRequest.post('/index.php',{c: 'User', action: 'userFinance', uid: that.$local.uid,link_url:location.href,rows:that.rows,page:that.page},function(res){
                    console.log("零钱明细")
                    console.log(res)
